...
impl<T> Readable<T> for ReadVolatile<T> {
    fn inner(&self) -> *const T {
        self.0
    }
}
...
impl<T> Writeable<T> for WriteVolatile<T> {
    unsafe fn inner(&mut self) -> *mut T {
        self.0
    }
}
...
impl<T> Readable<T> for Volatile<T> {
    fn inner(&self) -> *const T {
        self.0
    }
}
impl<T> Writeable<T> for Volatile<T> {
    unsafe fn inner(&mut self) -> *mut T {
        self.0
    }
}
impl<T> ReadableWriteable<T> for Volatile<T>
    where T: ::core::ops::BitAnd<Output = T>,
          T: ::core::ops::BitOr<Output = T> { }
...
impl<T> Readable<T> for UniqueVolatile<T> {
    fn inner(&self) -> *const T {
        self.0.as_ptr()
    }
}
impl<T> Writeable<T> for UniqueVolatile<T> {
    unsafe fn inner(&mut self) -> *mut T {
        self.0.as_ptr()
    }
}
impl<T> ReadableWriteable<T> for UniqueVolatile<T>
    where T: ::core::ops::BitAnd<Output = T>,
          T: ::core::ops::BitOr<Output = T> { }
