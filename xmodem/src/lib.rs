...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        if buf.len() < 128 {
            return Err(io::Error::new(io::ErrorKind::UnexpectedEof, "len < 128"));
        }

        let mut byte = self.read_byte(true)?;
        if byte != SOH {
            if byte == EOT {
                self.write_byte(NAK)?;
                self.expect_byte_or_cancel(EOT, "EOT expected")?;
                self.write_byte(ACK)?;
                return Ok(0);
            } else {
                self.inner.write(&[CAN])?;
        ...
        self.expect_byte_or_cancel(255 - self.packet, "")?;
        self.inner.read_max(buf)?;
